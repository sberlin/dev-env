...
Vagrant.configure("2") do |config|
  ...
  config.vm.provider "virtualbox" do |vb|
    ...
    vb.customize ["storageattach", :id, "--storagectl", "IDE Controller", "--device", "0", "--port", "0", "--nonrotational", "on", "--discard", "on"]
    #vb.customize ["storageattach", :id, "--storagectl", "SATA Controller", "--device", "0", "--port", "0", "--nonrotational", "on"]
  end
  ...
  config.vm.provision "ansible" do |ansible|
    ...
    ansible.extra_vars = {
      ...
      "password": vagrant_config["username"]
    }
  end
